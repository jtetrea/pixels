...
def _pip_install(packages: Sequence[str], *, no_deps: bool) -> None:
    ...
    if exit_code:
        raise RuntimeError(
            f"pip install failed with exit code {exit_code}: {list(packages)}"
        )
...
def _version_in_range(version: str, min_version: str, max_version: str) -> bool:
    ...
    return parsed >= _version_tuple(min_version) and parsed < _version_tuple(
        max_version
    )
...
def ensure_monai_runtime() -> None:
    ...
    Databricks AI Runtime v5 provides MLflow, Databricks Connect, PyTorch, and
    CUDA. This helper avoids downgrading those managed packages and only adds
    the MONAI Deploy / Holoscan / DICOM dependencies needed by the generated app.
    """
    ...
    try:
        ...
        raise RuntimeError(
            "Failed to activate holoscan-cu12 wheel-axle runtime"
        ) from exc
    importlib.invalidate_caches()
